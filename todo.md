...
## 新機能: 端末内OCRとAI分析方針
- [x] 画像・PDFをブラウザ内でOCRできるライブラリと既存アップロード画面の統合方法を調査する
- [x] 画像ファイルを端末内でOCRし、抽出テキストを報告書の下書きへ反映する
- [x] テキスト型PDFの端末内テキスト抽出を実装し、抽出テキストを下書きへ反映する
- [x] スキャンPDFをページ画像化して端末内OCRする対応を実装する
- [x] OCRの進捗・失敗表示と元ファイルのIndexedDB保存を実装する
- [x] OCR失敗時に同じ画像・PDFで再試行できる導線を追加する
- [x] OCRのローカル実行・抽出結果・オフライン利用をテストする
- [x] 個人情報を保護するAI分析の運用選択肢を文書化する

## 変更: ファイル選択のみでの自動下書き作成
- [x] 新規登録画面を画像/PDF選択主体のアップロード画面に変更する
- [x] OCR抽出テキストから下書きの事象概要・発生場所・対象者・報告種別を推定するローカル規則を実装する
- [x] OCR完了後に自動作成した下書きの確認・修正画面へ遷移する

## 新機能: 端末内OCRの向き補正
- [x] EXIF向き情報を反映し、画像を正しい向きで端末内OCRする
- [x] 向き情報がない画像・スキャンPDFを0/90/180/270度で判定して最適な向きに補正する
- [x] PDFをページ単位で向き補正・OCRし、進捗を表示する
- [x] PDF OCR失敗時に同じファイルで再試行できる導線を追加する

## バグ修正: 日本語OCRの自動下書き抽出
- [x] 文字間スペース・全角記号を正規化し、OCR抽出文から見出し項目と報告種別を正しく反映する
